...
async def _verify_user(username: str, password: str) -> bool:
    """Authenticate a network user (used by TACACS+ ASCII / PAP login).

    Constant-time: every code path performs *exactly one* bcrypt-equivalent
    operation (real or dummy) so the response time can't be used to
    enumerate usernames. The LDAP fallback is only invoked when the local
...
# Connection caps. The numbers are intentionally generous for legitimate
# operators (a busy data centre with 200 switches doing per-command
# authorization rarely needs more than ~50 concurrent sessions) but tight
# enough to bound the resource cost of a misconfigured or hostile peer.
# Override via :class:`naco.config.TacacsConfig` if you have a genuinely
# weird topology.
_MAX_TOTAL_CONNECTIONS_DEFAULT = 256
_MAX_PER_PEER_DEFAULT          = 32
...
async def run_tacacs_server() -> None:
    ...
    # Connection cap state: global semaphore + per-peer counter. The
    # semaphore enforces a hard ceiling on total simultaneous TCP sessions
    # (memory + FD bound); the per-peer counter prevents a single chatty
    # or malicious NAS from monopolising all the slots.
    max_total = getattr(cfg, "max_connections", _MAX_TOTAL_CONNECTIONS_DEFAULT)
    max_peer  = getattr(cfg, "max_connections_per_peer", _MAX_PER_PEER_DEFAULT)
    ...
    async def _client_cb(reader, writer):
        ...
        # Per-peer cap: cheap O(1) check before we take the global slot.
        async with peer_lock:
            current = peer_counts.get(peer_ip, 0)
            if current >= max_peer:
                log.warning(
                    "TACACS+ peer %s hit per-peer connection cap (%d) — dropping",
                    peer_ip, max_peer,
                )
                writer.close()
                try:
                    ...
                    pass
                return
            peer_counts[peer_ip] = current + 1

        # Global cap: acquire a semaphore slot for the lifetime of the
        # connection. ``acquire()`` blocks (cooperatively) when the cap is
        # exhausted — incoming connections back up in the kernel accept
        # queue, which is what we want; better to backpressure than to
        # OOM.
        ...
        try:
            ...
            total_sem.release()
            async with peer_lock:
                peer_counts[peer_ip] = max(0, peer_counts.get(peer_ip, 1) - 1)
                if peer_counts[peer_ip] == 0:
                    peer_counts.pop(peer_ip, None)

    server = await asyncio.start_server(_client_cb, cfg.host, cfg.port)
    log.info(
        "TACACS+ server listening on %s:%d (max %d connections, %d per peer)",
        cfg.host, cfg.port, max_total, max_peer,
    )
    try:
        async with server:
