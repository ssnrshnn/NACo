...
from naco.api.auth import (
    create_access_token, dummy_verify, get_current_admin,
    hash_password, needs_rehash, require_role, verify_password,
)
from naco.api.schemas import (
    ...
    StatusResponse, TacacsLogOut, TokenResponse, TotpVerifyRequest,
...
from naco.db.models import (
    ActiveSession, AdminRole, AdminUser, AdminAuditLog, AuthLog, AuthResult,
...
async def health_check(db: AsyncSession = Depends(get_db)):
    ...
    Public endpoint (no auth required) so load balancers and orchestrators can
    probe it without managing credentials.
    """
...
async def login(body: LoginRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    *before* any DB / bcrypt work so a locked-out account can't be used to
    ...
    # Otherwise the bcrypt cost (~250 ms at rounds=13) is a username-enumeration
    # oracle for anyone who can hit the login endpoint.
    if user is None:
    ...
    if needs_rehash(user.password_hash):
        ...
        db.add(AdminAuditLog(
            ...
            details={"reason": "bcrypt cost upgrade"},
        ))
...
async def totp_setup(
    ...
    The raw secret is **not** returned in JSON (only the ``otpauth://`` URI,
    which is required for authenticator apps). The pending value is activated
    by :func:`totp_verify` after the user proves possession with a valid code.
    """
    import pyotp
...
async def totp_verify(
    ...
    Reads the shared secret from ``admin.pending_totp_secret`` (set by
    ``/auth/totp/setup``). Never accepts ``secret`` as a query parameter —
    that leaked into access logs and Referer headers (Phase 0).
    """
    import pyotp
